/**
 * Download every Monday-attached file for the currently selected
 * profile-board patient. Mirrors the samantha-side button so the agent
 * can grab the prior round's clinicals before re-running Stedi.
 */
import { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { FolderDown, Loader2 } from "lucide-react";
import { fetchItemAssets } from "@/lib/profile/mondayApi";
...
export function ClinicalsDownloadButton({ itemId }: Props) {
  ...
  // Refresh the file count whenever the patient changes so the badge
  // reflects what's actually on Monday without requiring a click.
  useEffect(() => {
    let cancelled = false;
    if (!itemId) {
      setFileCount(null);
