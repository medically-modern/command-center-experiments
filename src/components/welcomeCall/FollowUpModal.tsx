/**
 * FollowUpModal — modal for marking a welcome-call patient as Follow Up.
 * Shows a date picker and writes Follow Up status + Follow Up Date to Monday.
 * Uses the same columns as CallAttemptsCounter (COL.followUp / COL.followUpDate).
 */
...
import { writeStatusIndex, writeDate, COL } from "@/lib/welcomeCall/mondayApi";
import { FOLLOW_UP_STATUS_INDEX } from "@/lib/welcomeCall/mondayWrite";
...
export function FollowUpModal({ open, onOpenChange, patientId, patientName, onSuccess }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await Promise.all([
        writeStatusIndex(patientId, COL.followUp, FOLLOW_UP_STATUS_INDEX),
